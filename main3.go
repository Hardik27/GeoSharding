...
// 	if len(peers) > 0 {
// 		net.Bootstrap("tcp://localhost:8001","tcp://localhost:8000","tcp://localhost:8002")
// 	}
...
func main() {
    ...
    for {
            ...
            for i := 0; i < len(clusters); i++ {
                for j := 0; j < len(clusters[i]); j++ {
                    if port != clusters[i][j] {
                        ...
                        nodeList[n]=node{port:port, shard:i+1, pubkey: keys.PublicKeyHex(), privatekey: keys.PrivateKeyHex()}
                        n++
                    }
                }
            }
        ...
        if time.Since(p)>30*time.Second{
            ...
            net.Broadcast(ctx,&messages.ClusterResponse{Port:int32(port),Shard:int32(max+1), Pubkey: keys.PublicKeyHex(), Privatekey: keys.PrivateKeyHex()})
            time.Sleep(15*time.Second)
            fmt.Println(nodeList)
